...
function mergeAccountConfig(base, override) {
	...
	for (const [key, value] of Object.entries(override)) {
		...
		if (value && typeof value === "object" && !Array.isArray(value) && baseVal && typeof baseVal === "object" && !Array.isArray(baseVal)) result[key] = {
			...baseVal,
			...value
		};
...
const DEFAULT_TTL_MS$2 = 1800 * 1e3;
var UserNameCache = class {
	map = /* @__PURE__ */ new Map();
	maxSize;
	ttlMs;
	constructor(maxSize = DEFAULT_MAX_SIZE$1, ttlMs = DEFAULT_TTL_MS$2) {
...
const log$26 = larkLogger("core/chat-info-cache");
...
var ChatInfoCache = class {
	...
	maxSize;
	ttlMs;
	constructor(maxSize = DEFAULT_MAX_SIZE, ttlMs = DEFAULT_TTL_MS$1) {
		this.maxSize = maxSize;
		this.ttlMs = ttlMs;
	}
	get(chatId) {
...
async function getChatInfo(params) {
	...
	try {
		...
		const data = (await _LarkClient.fromCfg(cfg, accountId).sdk.im.chat.get({ path: { chat_id: chatId } }))?.data;
		...
		log$26.info(`resolved ${chatId} → chat_mode=${chatMode}, group_message_type=${groupMessageType ?? "N/A"}`);
		return info;
	} catch (err) {
		log$26.error(`failed to get chat info for ${chatId}: ${String(err)}`);
		return;
	}
}
/**
...
*/
const log$25 = larkLogger("core/lark-client");
...
var LarkClient = class LarkClient {
	...
	*/
	static fromAccount(account) {
		const existing = cache$1.get(account.accountId);
		...
		if (existing) {
			log$25.info(`credentials changed, disposing stale instance`, { accountId: account.accountId });
			existing.dispose();
		}
		const instance = new LarkClient(account);
		cache$1.set(account.accountId, instance);
		return instance;
	}
	/**
	...
	*/
	...
	static get(accountId) {
		return cache$1.get(accountId) ?? null;
	}
	/**
	...
	*/
	static async clearCache(accountId) {
		if (accountId !== void 0) {
			cache$1.get(accountId)?.dispose();
			clearUserNameCache(accountId);
			clearChatInfoCache(accountId);
		} else {
			for (const inst of cache$1.values()) inst.dispose();
			clearUserNameCache();
			clearChatInfoCache();
		}
	}
	...
	*/
	async startWS(opts) {
		...
		const { appId, appSecret } = this.requireCredentials();
		if (this._wsClient) {
			log$25.warn(`closing previous WSClient before reconnect`, { accountId: this.accountId });
			try {
	...
	disconnect() {
		if (this._wsClient) {
			log$25.info(`disconnecting WebSocket`, { accountId: this.accountId });
			try {
				this._wsClient.close({ force: true });
			} catch {}
		}
		...
		if (this.messageDedup) {
			log$25.info(`disposing message dedup`, {
...
function getResolvedConfig(fallback) {
	try {
		...
		if (fallback?.channels?.feishu) {
			log$25.debug(`loadConfig() returned config without channels.feishu, using fallback`);
			return fallback;
		}
		return live;
	} catch {
...
//#endregion
//#region src/core/comment-target.ts
...
function buildFeishuCommentTarget(params) {
	...
	if (deliveryMode === "reply") return `${COMMENT_PREFIX}${params.fileType}:${params.fileToken}:${params.commentId}`;
...
function parseFeishuCommentTarget(target) {
	...
	let fileToken;
	let commentId;
	if (parts.length === 3) [fileType, fileToken, commentId] = parts;
	else if (parts.length === 4 && VALID_DELIVERY_MODES.has(parts[0])) [deliveryMode, fileType, fileToken, commentId] = parts;
	else return null;
...
*/
const log$24 = larkLogger("outbound/media");
/**
...
async function sendImageLark(params) {
	...
	log$24.info(`sendImageLark: target=${to}, imageKey=${imageKey}`);
	const client = LarkClient.fromCfg(cfg, accountId).sdk;
...
async function sendFileLark(params) {
	...
	log$24.info(`sendFileLark: target=${to}, fileKey=${fileKey}`);
	const client = LarkClient.fromCfg(cfg, accountId).sdk;
	return sendMediaMessage({
		client,
		to,
...
async function sendVideoLark(params) {
	const { cfg, to, fileKey, replyToMessageId, replyInThread, accountId } = params;
	log$24.info(`sendVideoLark: target=${to}, fileKey=${fileKey}`);
	const client = LarkClient.fromCfg(cfg, accountId).sdk;
	return sendMediaMessage({
		client,
		to,
...
async function sendAudioLark(params) {
	const { cfg, to, fileKey, replyToMessageId, replyInThread, accountId } = params;
	log$24.info(`sendAudioLark: target=${to}, fileKey=${fileKey}`);
	const client = LarkClient.fromCfg(cfg, accountId).sdk;
	return sendMediaMessage({
		client,
		to,
...
async function uploadAndSendMediaLark(params) {
	...
	log$24.info(`uploadAndSendMediaLark: target=${to}, source=${mediaBuffer ? "buffer" : mediaUrl ?? "(none)"}, fileName=${fileName ?? "(auto)"}`);
	let buffer;
	let resolvedFileName = fileName ?? "file";
	if (mediaBuffer) {
		buffer = mediaBuffer;
		log$24.debug(`using provided buffer: ${buffer.length} bytes`);
	} else if (mediaUrl) {
		buffer = await fetchMediaBuffer(mediaUrl, mediaLocalRoots);
		log$24.debug(`fetched media: ${buffer.length} bytes from "${mediaUrl}"`);
		if (!fileName) {
	...
	log$24.info(`resolved: fileName="${resolvedFileName}", type=${isImage ? "image" : "file"}, size=${buffer.length}`);
	if (isImage) {
		...
		log$24.debug(`image uploaded: imageKey=${imageKey}`);
	...
	log$24.debug(`file uploaded: fileKey=${fileKey}, fileType=${fileType}${isAudio || isVideo ? `, duration=${duration ?? "unknown"}ms` : ""}`);
...
async function validateRemoteUrl(raw) {
	...
	if (net.isIP(hostname)) {
		...
		log$24.warn(`[feishu-media] DNS resolution failed for "${hostname}": ${err}`);
	}
}
/**
...
async function fetchMediaBuffer(urlOrPath, localRoots) {
	...
	if (isLocalMediaPath(raw)) {
		...
		log$24.debug(`local file read: "${filePath}", ${buf.length} bytes`);
		return buf;
	}
	await validateRemoteUrl(raw);
	const FETCH_TIMEOUT_MS = 3e4;
	log$24.info(`fetching remote media: ${raw}`);
	...
	log$24.debug(`remote media fetched: ${raw}, ${arrayBuffer.byteLength} bytes`);
	return Buffer.from(arrayBuffer);
}
//#endregion
//#region src/messaging/outbound/deliver.ts
const log$23 = larkLogger("outbound/deliver");
/**
...
async function sendImMessage(params) {
	const { client, to, content, msgType, replyToMessageId, replyInThread } = params;
	if (replyToMessageId) {
		log$23.info(`replying to message ${replyToMessageId} (msg_type=${msgType}, thread=${replyInThread ?? false})`);
		const response = await client.im.message.reply({
			path: { message_id: replyToMessageId },
			data: {
				content,
	...
	log$23.debug(`message created: messageId=${result.messageId}`);
	return result;
}
/**
...
async function sendTextLark(params) {
	...
	if (card) {
		...
		log$23.info(`detected ${version} card JSON in text (target=${to}), routing to sendCardLark`);
		return sendCardLark({
			cfg,
			to,
			card,
	...
	log$23.info(`sendTextLark: target=${to}, textLength=${text.length}`);
	const client = LarkClient.fromCfg(cfg, accountId).sdk;
...
async function sendCardLark(params) {
	...
	const version = card.schema === "2.0" ? "v2" : "v1";
	log$23.info(`sendCardLark: target=${to}, cardVersion=${version}`);
	const client = LarkClient.fromCfg(cfg, accountId).sdk;
	...
	try {
		...
		log$23.error(`sendCardLark failed: ${detail}`);
...
async function sendMediaLark(params) {
	...
	log$23.info(`sendMediaLark: target=${to}, mediaUrl=${mediaUrl}`);
	try {
		...
		log$23.info(`media sent: messageId=${result.messageId}`);
		return {
		...
		log$23.error(`sendMediaLark failed for "${mediaUrl}": ${errMsg}`);
		log$23.info(`falling back to text link for "${mediaUrl}"`);
		return {
			...await sendTextLark({
				cfg,
				to,
...
async function sendCommentReplyLark(params) {
	...
	if (!target) throw new Error(`Not a valid comment target: "${to}"`);
	log$23.info(`sendCommentReplyLark: mode=${target.deliveryMode}, comment=${target.commentId}, textLength=${text.length}`);
	const client = LarkClient.fromCfg(cfg, accountId);
	const elements = [{
		type: "text_run",
		text_run: { text }
	}];
	if (target.deliveryMode === "create_whole") try {
		...
		log$23.info(`whole comment created successfully`);
		return {
		...
		const detail = formatLarkError(err);
		log$23.error(`sendCommentReplyLark failed to create whole comment: ${detail}`);
	...
	try {
		...
		log$23.info(`comment reply sent successfully`);
		return {
		...
		try {
			...
			log$23.info(`comment reply sent successfully (reply_elements fallback)`);
			return {
				messageId: `comment-reply:${target.commentId}`,
				chatId: to
			};
			...
			log$23.error(`sendCommentReplyLark failed: ${detail}`);
...
*/
function buildMarkdownCard$1(text) {
	return {
		schema: "2.0",
		config: { wide_screen_mode: true },
		body: { elements: [{
...
async function sendMarkdownCardFeishu(params) {
	...
	return sendCardFeishu({
		cfg,
		to,
		card: buildMarkdownCard$1(cardText),
		replyToMessageId,
		replyInThread,
		accountId
	});
...
const log$22 = larkLogger("core/app-scope-checker");
...
async function getAppGrantedScopes(sdk, appId, tokenType) {
	...
	try {
		...
		log$22.info(`fetched ${validScopes.length} scopes for app ${appId}`);
		const scopes = validScopes.filter((s) => {
			if (tokenType && s.token_types && Array.isArray(s.token_types)) return s.token_types.includes(tokenType);
			return true;
		}).map((s) => s.scope);
		log$22.info(`returning ${scopes.length} scopes${tokenType ? ` for ${tokenType} token` : ""}`);
		return scopes;
	} catch (err) {
		...
		log$22.warn(`failed to fetch scopes for ${appId}: ${err instanceof Error ? err.message : err}`);
		return [];
	}
}
/**
...
const log$21 = larkLogger("core/app-owner-fallback");
/**
...
async function getAppOwnerFallback(account, sdk) {
	...
	try {
		...
		log$21.warn(`failed to get owner for ${appId}: ${err instanceof Error ? err.message : err}`);
		return;
	}
}
//#endregion
...
const log$20 = larkLogger("core/device-flow");
/**
...
async function requestDeviceAuthorization(params) {
	...
	log$20.info(`requesting device authorization (scope="${scope}") url=${endpoints.deviceAuthorization} token_url=${endpoints.token}`);
	...
	log$20.info(`response status=${resp.status} body=${text.slice(0, 500)}`);
	let data;
	try {
	...
	log$20.info(`device_code obtained, expires_in=${expiresIn}s (${Math.round(expiresIn / 60)}min), interval=${interval}s`);
	return {
...
async function pollDeviceToken(params) {
	...
	while (Date.now() < deadline && attempts < MAX_POLL_ATTEMPTS) {
		...
		try {
			...
			log$20.warn(`poll network error: ${err}`);
			interval = Math.min(interval + 1, MAX_POLL_INTERVAL);
			continue;
		}
		const error = data.error;
		if (!error && data.access_token) {
			log$20.info("token obtained successfully");
			const refreshToken = data.refresh_token ?? "";
			const expiresIn = data.expires_in ?? 7200;
			let refreshExpiresIn = data.refresh_token_expires_in ?? 604800;
			if (!refreshToken) {
				log$20.warn("no refresh_token in response, token will not be refreshable");
		...
		if (error === "authorization_pending") {
			log$20.debug("authorization_pending, retrying...");
			continue;
		}
		if (error === "slow_down") {
			interval = Math.min(interval + 5, MAX_POLL_INTERVAL);
			log$20.info(`slow_down, interval increased to ${interval}s`);
			continue;
		}
		if (error === "access_denied") {
			log$20.info("user denied authorization");
			return {
				ok: false,
				error: "access_denied",
				message: "用户拒绝了授权"
		...
		if (error === "expired_token" || error === "invalid_grant") {
			log$20.info(`device code expired/invalid (error=${error})`);
			return {
				ok: false,
				error: "expired_token",
		...
		log$20.warn(`unexpected error: error=${error}, desc=${desc}`);
		return {
			ok: false,
			error: "expired_token",
			message: desc
	...
	if (attempts >= MAX_POLL_ATTEMPTS) log$20.warn(`max poll attempts (${MAX_POLL_ATTEMPTS}) reached`);
	return {
		ok: false,
		error: "expired_token",
		message: "授权超时，请重新发起"
...
*/
const log$19 = larkLogger("core/token-store");
const execFile = promisify(createRequire(typeof __filename !== "undefined" ? __filename : import.meta.url)(`node:${["child", "process"].join("_")}`).execFile);
...
async function getMasterKey() {
	try {
		...
		log$19.warn("master key has unexpected length, regenerating");
	} catch (err) {
		if (!(err instanceof Error) || err.code !== "ENOENT") log$19.warn(`failed to read master key: ${err instanceof Error ? err.message : err}`);
	}
	await ensureLinuxCredDir();
	const key = randomBytes(MASTER_KEY_BYTES);
	await writeFile(MASTER_KEY_PATH, key, { mode: 384 });
	await chmod(MASTER_KEY_PATH, 384);
	log$19.info("generated new master key for encrypted file storage");
	return key;
}
...
async function getWin32MasterKey() {
	try {
		...
		if (key.length === MASTER_KEY_BYTES) return key;
		log$19.warn("win32 master key has unexpected length, regenerating");
	} catch (err) {
		if (!(err instanceof Error) || err.code !== "ENOENT") log$19.warn(`failed to read win32 master key: ${err instanceof Error ? err.message : err}`);
	}
	await ensureWin32CredDir();
	const key = randomBytes(MASTER_KEY_BYTES);
	await writeFile(WIN32_MASTER_KEY_PATH, key);
	log$19.info("generated new master key for win32 encrypted file storage");
	return key;
}
const win32Backend = {
	async get(_service, account) {
...
function createBackend() {
	switch (process.platform) {
		...
		default:
			log$19.warn(`unsupported platform "${process.platform}", falling back to macOS backend`);
...
async function setStoredToken(token) {
	...
	log$19.info(`saved UAT for ${token.userOpenId} (at:${maskToken(token.accessToken)})`);
}
/**
...
async function removeStoredToken(appId, userOpenId) {
	...
	log$19.info(`removed UAT for ${userOpenId}`);
}
/**
...
const log$18 = larkLogger("core/uat-client");
/**
...
async function doRefreshToken(opts, stored) {
	if (Date.now() >= stored.refreshExpiresAt) {
		log$18.info(`refresh_token expired for ${opts.userOpenId}, clearing`);
	...
	if (code !== void 0 && code !== 0 || error) {
		...
		if (REFRESH_TOKEN_RETRYABLE.has(code)) {
			log$18.warn(`refresh transient error (code=${errCode}) for ${opts.userOpenId}, retrying once`);
			...
			if (retryCode !== void 0 && retryCode !== 0 || retryError) {
				...
				log$18.warn(`refresh failed after retry (code=${retryErrCode}), clearing token for ${opts.userOpenId}`);
				await removeStoredToken(opts.appId, opts.userOpenId);
				return null;
			}
		} else {
			log$18.warn(`refresh failed (code=${errCode}), clearing token for ${opts.userOpenId}`);
			await removeStoredToken(opts.appId, opts.userOpenId);
			return null;
		}
	}
	...
	log$18.info(`refreshed UAT for ${opts.userOpenId} (at:${maskToken(updated.accessToken)})`);
	return updated;
}
/**
...
async function callWithUAT(opts, apiCall) {
	...
	try {
		...
		if (TOKEN_RETRY_CODES.has(code)) {
			log$18.warn(`API call failed (code=${code}), refreshing and retrying`);
			const stored = await getStoredToken(opts.appId, opts.userOpenId);
			if (!stored) throw new NeedAuthorizationError(opts.userOpenId);
			const refreshed = await refreshWithLock(opts, stored);
			if (!refreshed) throw new NeedAuthorizationError(opts.userOpenId);
...
async function revokeUAT(appId, userOpenId) {
	...
	log$18.info(`revoked UAT for ${userOpenId}`);
}
//#endregion
//#region src/card/card-error.ts
/**
* Copyright (c) 2026 ByteDance Ltd. and/or its affiliates
* SPDX-License-Identifier: MIT
*
...
const CARD_CONTENT_SUB_ERROR = { ELEMENT_LIMIT: 11310 };
/** CardKit API 返回非零 code 时的结构化错误。 */
var CardKitApiError = class extends Error {
	code;
	msg;
	constructor(params) {
...
function parseCardApiError(err) {
	...
	if (err && typeof err === "object") {
		const e = err;
		if (typeof e.msg === "string") errMsg = e.msg;
		else if (typeof e.response?.data?.msg === "string") errMsg = e.response.data.msg;
		else if (typeof e.message === "string") errMsg = e.message;
	}
	return {
		code,
...
function isCardTableLimitError(err) {
	...
	return parsed.code === CARD_ERROR.CARD_CONTENT_FAILED && parsed.subCode === CARD_CONTENT_SUB_ERROR.ELEMENT_LIMIT && /table number over limit/i.test(parsed.errMsg);
}
...
function findMarkdownTablesOutsideCodeBlocks(text) {
	...
	while (tableMatch != null) {
		if (!isInsideCodeBlock(tableMatch.index)) matches.push({
...
* 作为同一张卡片渲染的多块文本。
*/
function sanitizeTextSegmentsForCard(texts, tableLimit = 3) {
...
function enqueueFeishuChatTask(params) {
	...
	const cleanup = () => {
		if (chatQueues.get(key) === taskPromise) chatQueues.delete(key);
	};
...
//#endregion
//#region src/messaging/inbound/handler-registry.ts
let inboundHandler = null;
function injectInboundHandler(handler) {
	inboundHandler = handler;
}
function getInboundHandler() {
	if (!inboundHandler) throw new Error("Feishu inbound handler has not been initialised.");
	return inboundHandler;
}
//#endregion
//#region src/messaging/inbound/synthetic-message.ts
...
const log$16 = larkLogger("tools/oauth");
const json$1 = formatToolResult;
...
async function verifyTokenIdentity(brand, accessToken, expectedOpenId) {
	...
	try {
		...
		if (data.code !== 0) {
			log$16.warn(`user_info API error: code=${data.code}, msg=${data.msg}`);
			return { valid: false };
		}
		const actualOpenId = data.data?.open_id;
		if (!actualOpenId) {
			log$16.warn("user_info API returned no open_id");
			return { valid: false };
		}
		return {
		...
		log$16.warn(`identity verification request failed: ${err}`);
		return { valid: false };
	}
}
function registerFeishuOAuthTool(api) {
	...
	registerTool(api, {
		...
		async execute(_toolCallId, params) {
			...
			if (!senderOpenId) return json$1({ error: "无法获取当前用户身份（senderOpenId），请在飞书对话中使用此工具。" });
			const acct = getLarkAccount(cfg, ticket.accountId);
			if (!acct.configured) return json$1({ error: `账号 ${ticket.accountId} 缺少 appId 或 appSecret 配置` });
			const account = acct;
			try {
				switch (p.action) {
					case "revoke":
					...
					default: return json$1({ error: `未知操作: ${p.action}` });
				}
			} catch (err) {
				log$16.error(`${p.action} failed: ${err}`);
				return json$1({ error: formatLarkError(err) });
			}
		}
	}, { name: "feishu_oauth" });
...
async function executeAuthorize(params) {
	...
	try {
		...
		if (err instanceof OwnerAccessDeniedError) {
			log$16.warn(`non-owner user ${senderOpenId} attempted to authorize`);
			return json$1({
	...
	if (existing && tokenStatus(existing) !== "expired") if (effectiveScope) {
		...
		if (missingScopes.length > 0) log$16.info(`existing token missing scopes [${missingScopes.join(", ")}], starting incremental auth`);
		else {
			if (onAuthComplete) try {
				await onAuthComplete();
			} catch (e) {
				log$16.warn(`onAuthComplete failed: ${e}`);
			}
			return json$1({
				success: true,
	...
	if (pendingFlows.has(flowKey)) {
		...
		if (oldFlow.messageId === currentMessageId) {
			...
			if (oldFlow.scope) {
				...
				log$16.info(`scope merge on reuse: [${[...merged].join(", ")}]`);
			}
			log$16.info(`same message, replacing flow for user=${senderOpenId}, app=${appId}, reusing cardId=${reuseCardId}`);
		} else {
			oldFlow.superseded = true;
			oldFlow.controller.abort();
			pendingFlows.delete(flowKey);
			log$16.info(`new message, cancelling old flow for user=${senderOpenId}, app=${appId}, old cardId=${oldFlow.cardId}`);
			try {
				...
				log$16.warn(`failed to update old card to expired: ${e}`);
			}
		}
	}
	...
	if (effectiveScope) try {
		...
		if (unavailableScopes.length > 0) {
			...
			log$16.info(`proceeding with available scopes [${availableScopes.join(", ")}]`);
		}
	} catch (err) {
		log$16.warn(`failed to check app scopes, proceeding anyway: ${err}`);
	}
	...
	if (!chatId || !ticket) return json$1({ error: "无法确定发送目标" });
	if (reuseCardId) {
		...
		try {
			...
			log$16.info(`updated existing card ${reuseCardId} with merged scopes, seq=${newSeq}`);
		} catch (err) {
			log$16.warn(`failed to update existing card, creating new one: ${err}`);
			const newCardId = await createCardEntity({
				cfg,
				card: authCard,
				accountId
			});
			if (!newCardId) return json$1({ error: "创建授权卡片失败" });
	...
	pollDeviceToken({
		...
		if (currentFlow.superseded) {
			log$16.info(`flow superseded, skipping card update for cardId=${cardId}`);
			return;
		}
		if (result.ok) {
			const identity = await verifyTokenIdentity(brand, result.token.accessToken, senderOpenId);
			if (!identity.valid) {
				log$16.warn(`identity mismatch! expected=${senderOpenId}, actual=${identity.actualOpenId ?? "unknown"}, cardId=${cardId}`);
				try {
					await updateCardKitCardForAuth({
						cfg,
						cardId,
					...
					log$16.warn(`failed to update card for identity mismatch: ${e}`);
				}
				pendingFlows.delete(flowKey);
			...
			try {
				...
				log$16.warn(`failed to update card to success: ${e}`);
			}
			pendingFlows.delete(flowKey);
			pendingFlowDelete = true;
			if (onAuthComplete) try {
			...
			if (skipSyntheticMessage) log$16.info("skipSyntheticMessage=true, skipping synthetic message");
			else try {
				const status = await dispatchSyntheticTextMessage({
					...
					syntheticMessageId: `${ticket.messageId}:auth-complete`,
					replyToMessageId: ticket.messageId,
					chatType: ticket.chatType,
					threadId: ticket.threadId,
					runtime: {
						log: (msg) => log$16.info(msg),
						error: (msg) => log$16.error(msg)
					}
				});
				log$16.info(`synthetic message queued (${status})`);
				log$16.info("synthetic message dispatched after successful auth");
			} catch (e) {
				log$16.warn(`failed to send synthetic message after auth: ${e}`);
			}
		} else {
			try {
				await updateCardKitCardForAuth({
				...
				log$16.warn(`failed to update card to failure: ${e}`);
			}
			pendingFlows.delete(flowKey);
			pendingFlowDelete = true;
		}
	}).catch((err) => {
		log$16.error(`polling error: ${err}`);
	}).finally(() => {
...
const log$15 = larkLogger("tools/onboarding-auth");
...
async function triggerOnboarding(params) {
	...
	if (!acct.configured) {
		log$15.warn(`account ${accountId} not configured, skipping`);
		return;
	}
	...
	if (!ownerOpenId) {
		log$15.info(`app ${appId} has no owner info, skipping`);
		return;
	}
	if (userOpenId !== ownerOpenId) {
		log$15.info(`user ${userOpenId} is not app owner (${ownerOpenId}), skipping`);
		return;
	}
	log$15.info(`user ${userOpenId} is app owner, starting OAuth`);
	let allUserScopes;
	try {
		allUserScopes = await getAppGrantedScopes(sdk, appId, "user");
	} catch (err) {
		log$15.warn(`failed to get app granted scopes: ${err}`);
		return;
	}
	allUserScopes = filterSensitiveScopes(allUserScopes);
	if (allUserScopes.length === 0) {
		log$15.info("no user scopes configured, skipping");
		return;
	}
	const batches = [];
	for (let i = 0; i < allUserScopes.length; i += MAX_SCOPES_PER_BATCH) batches.push(allUserScopes.slice(i, i + MAX_SCOPES_PER_BATCH));
	log$15.info(`${allUserScopes.length} user scopes, ${batches.length} batch(es)`);
	const startBatch = async (batchIndex) => {
		if (batchIndex >= batches.length) {
			log$15.info("all batches completed");
			return;
		}
		...
		log$15.info(`starting batch ${batchIndex + 1}/${batches.length}, scopes=${batch.length}`);
		try {
			await executeAuthorize({
				...
				onAuthComplete: async () => {
					log$15.info(`batch ${batchIndex + 1}/${batches.length} auth completed`);
					await startBatch(batchIndex + 1);
				}
			});
		} catch (err) {
			log$15.error(`batch ${batchIndex + 1} failed: ${err}`);
		}
	};
	await startBatch(0);
}
...
const log$14 = larkLogger("tools/auto-auth");
...
function enqueueAuthRequest(bufferKey, scopes, ctx, flushFn, debounceMs = AUTH_DEBOUNCE_MS) {
	...
	if (existing) {
		...
		if (existing.phase === "executing") {
			log$14.info(`auth in-flight, piggyback → key=${bufferKey}, scopes=[${[...existing.scopes].join(", ")}]`);
			...
			existing.updateTimer = setTimeout(async () => {
				...
				if (existing.isUpdating) {
					...
					log$14.info(`scope update deferred (previous update still running) → key=${bufferKey}`);
					return;
				}
				existing.isUpdating = true;
				try {
					const mergedScopes = [...existing.scopes];
					log$14.info(`scope update flush → key=${bufferKey}, scopes=[${mergedScopes.join(", ")}]`);
					await existing.flushFn(mergedScopes);
				} catch (err) {
					log$14.warn(`scope update failed: ${err}`);
				} finally {
					existing.isUpdating = false;
					if (existing.pendingReupdate) {
						existing.pendingReupdate = false;
						const finalScopes = [...existing.scopes];
						log$14.info(`scope reupdate → key=${bufferKey}, scopes=[${finalScopes.join(", ")}]`);
						try {
							await existing.flushFn(finalScopes);
						} catch (err) {
							log$14.warn(`scope reupdate failed: ${err}`);
						}
					}
				}
		...
		log$14.info(`debounce merge → key=${bufferKey}, scopes=[${[...existing.scopes].join(", ")}]`);
		return new Promise((resolve, reject) => {
	...
	entry.timer = setTimeout(async () => {
		...
		log$14.info(`debounce flush → key=${bufferKey}, waiters=${entry.waiters.length}, scopes=[${mergedScopes.join(", ")}]`);
...
function addToDeferredUserAuth(ticket, scopes, account, cfg) {
	...
	if (existing) {
		for (const s of scopes) existing.scopes.add(s);
		log$14.info(`deferred user auth scope merge → key=${key}, scopes=[${[...existing.scopes].join(", ")}]`);
	} else {
		...
		log$14.info(`deferred user auth created → key=${key}, scopes=[${scopes.join(", ")}]`);
	}
}
/** v2 卡片 i18n 配置 */
const I18N_CONFIG = {
...
async function sendAppScopeCard(params) {
	...
	if (appAuthFlows.getByDedupKey(dedup)) {
		log$14.info(`dedup – app-scope card already pending for chatId=${chatId}, scopes=[${missingScopes.join(", ")}], skipping duplicate send`);
		return json({
	...
	if (activeEntry) {
		...
		if (!migrated) log$14.info(`migrate raced, falling through to new card creation`);
		else try {
			await updateCardKitCardForAuth({
				cfg,
			...
			log$14.info(`app-scope card updated in-place, cardId=${activeFlow.cardId}, seq=${newSeq}, scopes=[${missingScopes.join(", ")}]`);
			...
			log$14.warn(`failed to update existing app-scope card, creating new one: ${err}`);
		}
	}
	...
	if (!cardId) {
		log$14.warn("createCardEntity failed for app-scope card, falling back");
		return json({
	...
	log$14.info(`app-scope card sent, operationId=${operationId}, scopes=[${missingScopes.join(", ")}]`);
	return json({
		awaiting_app_authorization: true,
		message: "已向用户发送授权引导卡片，等待用户完成授权操作。请告知用户：按照卡片提示完成授权，完成后系统将自动重试之前的操作。请等待用户完成卡片操作，不要建议其他替代方案。",
		missing_scopes: missingScopes
...
async function handleCardAction(data, cfg, accountId) {
	...
	if (!flow) {
		log$14.warn(`card action ${operationId} not found (expired or already handled)`);
		return;
	}
	log$14.info(`app_auth_done clicked by ${senderOpenId}, operationId=${operationId}`);
	invalidateAppScopeCache(flow.appId);
	const acct = getLarkAccount(flow.cfg, flow.accountId);
	if (!acct.configured) {
		log$14.warn(`account ${flow.accountId} not configured, skipping OAuth`);
		return;
	}
	const sdk = LarkClient.fromAccount(acct).sdk;
	let grantedScopes = [];
	try {
		...
		log$14.warn(`failed to re-check app scopes: ${err}, proceeding anyway`);
	}
	if (!isAppScopeSatisfied(grantedScopes, flow.requiredScopes, flow.scopeNeedType)) {
		log$14.warn(`app scopes still missing after user confirmation: [${flow.requiredScopes.join(", ")}]`);
		return { toast: {
			type: "error",
			content: "权限尚未开通，请确认已申请并审核通过后再试"
		} };
	}
	log$14.info(`app scopes verified, proceeding with OAuth`);
	...
	if (consumedDeferred && deferKey) {
		deferredUserAuth.delete(deferKey);
		log$14.info(`consumed deferred user auth scopes: [${[...consumedDeferred.scopes].join(", ")}]`);
	}
	...
	setImmediate(async () => {
		try {
			try {
				...
				log$14.warn(`failed to update app-scope card to progress via API: ${err}`);
			}
			if (!flow.ticket.senderOpenId) {
				log$14.warn("no senderOpenId in ticket, skipping OAuth");
				return;
			}
			...
			if (userBatch) {
				...
				log$14.info(`merged user batch scopes into app auth completion: [${[...mergedScopes].join(", ")}]`);
			}
			if (mergedScopes.size === 0) {
				log$14.info("no business scopes to authorize after app auth, sending synthetic message for retry");
				...
				await dispatchSyntheticTextMessage({
					...
					runtime: {
						log: (msg) => log$14.info(msg),
						error: (msg) => log$14.error(msg)
					}
				});
				log$14.info("synthetic message dispatched after app-auth-only completion");
			...
			log$14.error(`handleCardAction background task failed: ${err}`);
		}
	});
	return {
		toast: {
...
async function handleInvokeErrorWithAutoAuth(err, cfg) {
	...
	if (ticket) {
				...
				try {
					...
					if (acct.configured) {
						if (hasActiveAppAuthForMessage(ticket)) {
							...
							log$14.info(`UserAuthRequiredError deferred (app auth pending), scopes=[${scopes.join(", ")}]`);
							return json({
								awaiting_app_authorization: true,
						...
						log$14.info(`UserAuthRequiredError → enqueue, key=${bufferKey}, scopes=[${scopes.join(", ")}]`);
					...
					log$14.warn(`executeAuthorize failed: ${autoAuthErr}, falling back`);
				}
			}
			if (err instanceof UserScopeInsufficientError) {
				...
				try {
					...
					if (acct.configured) {
						if (hasActiveAppAuthForMessage(ticket)) {
							addToDeferredUserAuth(ticket, scopes, acct, cfg);
							log$14.info(`UserScopeInsufficientError deferred (app auth pending), scopes=[${scopes.join(", ")}]`);
							return json({
								awaiting_app_authorization: true,
								user_auth_deferred: true,
								message: "应用权限尚未开通，将在应用权限通过后自动为您发起用户授权。请先按照应用权限卡片的提示完成操作。请等待用户完成卡片操作，不要建议其他替代方案。",
						...
						const bufferKey = `user:${ticket.accountId}:${senderOpenId}:${ticket.messageId}`;
						log$14.info(`UserScopeInsufficientError → enqueue, key=${bufferKey}, scopes=[${scopes.join(", ")}]`);
						return await enqueueAuthRequest(bufferKey, scopes, {
							account: acct,
							cfg,
							ticket
		...
		} else log$14.error(`senderOpenId not found ${err}`);
			...
			try {
				...
				if (acct.configured) {
					if (senderOpenId && appScopeErr.allRequiredScopes?.length) {
						...
						log$14.info(`AppScopeMissingError → deferred allRequiredScopes=[${appScopeErr.allRequiredScopes.join(", ")}]`);
					}
					const bufferKey = `app:${ticket.accountId}:${ticket.chatId}:${ticket.messageId}`;
					log$14.info(`AppScopeMissingError → enqueue, key=${bufferKey}, scopes=[${appScopeErr.missingScopes.join(", ")}]`);
				...
				log$14.warn(`sendAppScopeCard failed: ${cardErr}, falling back`);
			}
		}
	} else log$14.error(`ticket not found ${err}`);
...
//#endregion
//#region src/messaging/converters/content-converter-helpers.ts
...
*/
const log$13 = larkLogger("converters/merge-forward");
/**
...
async function expand(accountId, messageId, resolveUserName, batchResolveNames, fetchSubMessages, convertContent) {
	...
	try {
		...
		log$13.error("fetch sub-messages failed", {
			messageId,
			error: error instanceof Error ? error.message : String(error)
		});
	...
	if (senderIds.length > 0 && batchResolveNames) try {
		...
		log$13.debug("batchResolveNames failed (best-effort)", { error: err instanceof Error ? err.message : String(err) });
	}
...
async function formatSubTree(parentId, childrenMap, accountId, resolveUserName, convertContent) {
	...
	for (const item of children) try {
		...
		if (msgType === "merge_forward") {
			...
			if (nestedId) content = await formatSubTree(nestedId, childrenMap, accountId, resolveUserName, convertContent);
			...
			if (!convertContent) content = rawContent;
			else content = (await convertContent(rawContent, msgType, subCtx)).content;
		}
		const displayName = resolveUserName?.(senderId) ?? senderId;
		const indented = indentLines(content, "    ");
		parts.push(`[${timestamp}] ${displayName}:\n${indented}`);
	} catch (err) {
		log$13.warn("failed to convert sub-message", {
...
async function convertMessageContent(raw, messageType, ctx) {
	...
	return fn(raw, ctx.convertMessageContent ? ctx : {
		...ctx,
...
const log$12 = larkLogger("inbound/parse-io");
/**
...
async function fetchCardContent(messageId, larkClient) {
	try {
		...
		log$12.warn(`fetchCardContent failed for ${messageId}: ${err instanceof Error ? err.message : String(err)}`);
		return;
	}
}
/**
...
function createParseResolveNames(account) {
	return createBatchResolveNames(account, (...args) => log$12.info(args.map(String).join(" ")));
}
//#endregion
//#region src/messaging/inbound/parse.ts
const log$11 = larkLogger("inbound/parse");
/**
...
async function parseMessageEvent(event, botOpenId, expandCtx) {
	...
	if (event.message.message_type === "interactive" && expandCtx) {
		...
		if (fullContent) {
			...
			log$11.info("replaced interactive content with full v2 card data");
		}
	}
	const convertCtx = {
		mentions: mentionMap,
...
const log$10 = larkLogger("shared/message-lookup");
/**
...
async function getMessageFeishu(params) {
	...
	try {
		...
		if (!items || items.length === 0) {
			log$10.info(`getMessageFeishu: no items returned for ${messageId}`);
			return null;
		}
		const expandCtx = expandForward ? {
			...
			batchResolveNames: createBatchResolveNames(getLarkAccount(cfg, accountId), (...args) => log$10.info(args.map(String).join(" ")))
		} : void 0;
		return await parseMessageItem(items[0], messageId, expandCtx);
	} catch (error) {
		log$10.error(`get message failed (${messageId}): ${error instanceof Error ? error.message : String(error)}`);
		return null;
	}
}
/**
...
function resolveRespondToMentionAll(params) {
	return params.groupConfig?.respondToMentionAll ?? params.defaultConfig?.respondToMentionAll ?? params.accountFeishuCfg?.respondToMentionAll ?? false;
}
...
const log$9 = larkLogger("outbound/typing");
/**
...
async function addTypingIndicator(params) {
	...
	try {
		...
		if (isMessageUnavailableError(error)) {
			log$9.debug(`Skip add typing indicator for unavailable message`, { messageId: normalizedId });
			return state;
		}
		log$9.debug(`Failed to add typing indicator`, {
			messageId,
...
async function removeTypingIndicator(params) {
	...
	try {
		...
		if (isMessageUnavailableError(error)) {
			log$9.debug(`Skip remove typing indicator for unavailable message`, { messageId: state.messageId });
			return;
		}
		log$9.debug(`Failed to remove typing indicator`, {
			messageId: state.messageId,
			error: error instanceof Error ? error.message : error
		});
	}
...
const DEFAULT_SUMMARY_PREFERENCE = [
	"matched",
	"code",
	"quoted",
	"url",
...
function normalizeToolUseDisplay(params) {
	...
	const steps = traceSteps.map(toTraceSource).map((source) => formatToolStep(source, {
		showFullPaths,
		showResultDetails
	})).filter((step) => !!step);
	return {
...
function formatToolStep(source, options) {
	...
	const rawDetail = (descriptor ? extractDetailFromParams(source.params, descriptor) : void 0) ?? (descriptor ? extractDetailFromSummary(source.summaryText, descriptor) : cleanupLine(source.summaryText ?? "")) ?? void 0;
	const detail = rawDetail ? sanitizeToolDetail(descriptor?.sanitizer ?? "generic", rawDetail, options) : void 0;
	const title = buildToolTitle(source, descriptor, rawDetail);
	const status = resolveStepStatus(source);
	const errorBlock = source.error ? buildErrorBlock(source.error, descriptor) : void 0;
	const resultBlock = !errorBlock && options.showResultDetails ? buildResultBlock(source, descriptor) : void 0;
	return {
		title,
		detail,
...
function buildToolTitle(source, descriptor, rawDetail) {
	const baseTitle = descriptor?.title === "Read" && rawDetail && isSkillPathValue(rawDetail) ? "Skill Read" : descriptor?.title ?? humanizeToolName(source.toolName ?? "tool");
	const durationLabel = source.durationMs != null ? formatDurationLabel(source.durationMs) : void 0;
...
function extractDetailFromSummary(summaryText, descriptor) {
	...
	const lines = summaryText.replace(/\r\n/g, "\n").split("\n").map((line) => cleanupLine(stripMarkdown(line))).filter((line) => line && !isNoiseLine(line));
	for (const line of lines) {
		const detail = pickSummaryDetail(buildSummarySignals(line, descriptor.summaryPatterns ?? []), descriptor.summaryPreference ?? DEFAULT_SUMMARY_PREFERENCE);
...
function buildSummarySignals(line, patterns) {
	return {
		line,
		matched: patterns.map((pattern) => line.match(pattern)?.[1]?.trim()).find((value) => Boolean(value)),
...
function buildResultBlock(source, descriptor) {
	if (source.result == null) return void 0;
	if (descriptor && [
		"Read",
		"Edit",
		"Fetch web page",
		"Browser"
	].includes(descriptor.title)) return;
...
function sanitizeDisplayBlockValue(value, descriptor) {
	if (descriptor?.sanitizer === "command" && typeof value === "string") return redactInlineSecrets(value);
	return value;
}
function buildPatternDetail(params, options) {
	...
	if (pattern && target && options.includeTarget) return `${pattern} in ${target}`;
	return pattern ?? target ?? void 0;
}
...
function sanitizeToolDetail(kind, value, options) {
	...
	if (!cleaned) return void 0;
	switch (kind) {
		case "skill": return cleaned.replace(/^skill\s+/i, "").replace(/[-_]+/g, " ").trim() || "skill";
		case "path": return sanitizePathLike(cleaned, options);
		case "search": return stripQuotes(cleaned);
		case "url": return stripQuotes(cleaned).replace(/^from\s+/i, "");
...
function sanitizePathLike(value, options) {
	...
	if (skillMatch?.[1]) return skillMatch[1].replace(/[-_]+/g, " ").trim() || cleaned;
	return cleaned.split(/[\\/]/).filter(Boolean).at(-1) ?? cleaned;
}
function sanitizeCommandLike(value, options) {
	const cleaned = stripQuotes(value).replace(/^(?:command|script|description)\s+/i, "").replace(/^.*?\s+->\s+/i, "").trim();
...
function redactCommandPaths(command) {
	return command.split(/(\s+)/).map((segment) => {
...
function redactCommandToken(token) {
	...
	return `${prefix}${redactPathAssignment(rawCore)}${suffix}`;
}
function redactPathAssignment(value) {
	const equalsIndex = value.indexOf("=");
	if (equalsIndex > 0) return `${value.slice(0, equalsIndex + 1)}${redactStandalonePath(value.slice(equalsIndex + 1))}`;
...
function sanitizeUrlForDisplay(url) {
	try {
		...
		parsed.username = "";
		parsed.password = "";
		for (const key of [...parsed.searchParams.keys()]) if (/(secret|token|password|key|credential|bearer|auth)/i.test(key)) parsed.searchParams.set(key, "[redacted]");
...
function basenameFromPath(value) {
	return value.replace(/\\/g, "/").replace(/\/+$/, "").split("/").filter(Boolean).at(-1) ?? value;
}
...
function stripMarkdown(line) {
	return line.replace(/`([^`]+)`/g, "$1").replace(/\*\*([^*]+)\*\*/g, "$1").replace(/\*([^*]+)\*/g, "$1").replace(/^>\s*/, "").trim();
}
...
function extractFirstCodeSpan(value) {
	return value.match(/`([^`]+)`/)?.[1]?.trim() || void 0;
}
function extractFirstQuotedText(value) {
	return value.match(/["']([^"']+)["']/)?.[1]?.trim() || void 0;
}
function extractFirstUrl(value) {
	return value.match(/\bhttps?:\/\/[^\s"'`]+/i)?.[0]?.trim() || void 0;
}
//#endregion
...
function formatFooterRuntimeSegments(params) {
	...
	if (footer?.tokens && metrics) {
		...
		const outTokens = typeof metrics.outputTokens === "number" ? Math.max(0, metrics.outputTokens) : void 0;
	...
	if (footer?.cache && metrics) {
		const read = typeof metrics.cacheRead === "number" ? Math.max(0, metrics.cacheRead) : void 0;
		const write = typeof metrics.cacheWrite === "number" ? Math.max(0, metrics.cacheWrite) : void 0;
		const inputVal = typeof metrics.inputTokens === "number" ? Math.max(0, metrics.inputTokens) : void 0;
	...
	if (footer?.context && metrics) {
		const freshTotal = metrics.totalTokensFresh === false ? void 0 : metrics.totalTokens;
		...
		if (total != null && ctx != null) {
			...
			const pctLabel = `${ctx > 0 ? Math.round(total / ctx * 100) : 0}%`;
...
function buildCardContent(state, data = {}) {
	switch (state) {
		...
		case "streaming": return buildStreamingCard(data.text ?? "", {
...
function buildStreamingCard(partialText, params = {}) {
	...
	if (showToolUse) elements.push(hasToolUse ? buildToolUsePanel({
		toolUseSteps,
...
function buildCompleteCard(params) {
	...
	const elements = [];
	if (showToolUse) elements.push(buildToolUsePanel({
		toolUseSteps,
		toolUseElapsedMs,
		titleSuffix: toolUseTitleSuffix
	}));
	...
	if (footerZhLines.length > 0) elements.push(...buildFooter(footerZhLines.join("\n"), footerEnLines.join("\n"), isError));
...
function buildStreamingPreAnswerCard(params) {
	...
	const elements = [];
	if (showToolUse) elements.push(hasSteps ? buildStreamingToolUseActivePanel({
		steps,
		elapsedMs
	}) : buildStreamingToolUsePendingPanel());
	elements.push({
...
function buildToolUsePanel(params) {
	...
	const stepElements = toolUseSteps.length > 0 ? toolUseSteps.flatMap((step) => buildToolUseStepElements(step)) : [buildToolUsePlaceholder()];
	return {
		tag: "collapsible_panel",
		expanded: false,
		header: {
...
function buildToolUsePlaceholder(labels) {
	...
	const en = labels?.en ?? "No tool steps available";
	return {
		tag: "div",
		text: {
			tag: "plain_text",
...
function buildToolUseStepTitleMarkdown(step) {
	...
	return optimizeMarkdownStyle(`**${escapeToolUseMarkdownText(step.title)}** · <font color='${status.color}'>${status.label}</font>`, 1);
}
...
function longestBacktickRun(value) {
	return (value.match(/`+/g) ?? []).reduce((max, run) => Math.max(max, run.length), 0);
}
...
function shouldUseCard(text) {
	...
	if (tableMatches.length > 3) return false;
	if (/```[\s\S]*?```/.test(text)) return true;
	if (tableMatches.length > 0) return true;
	return false;
}
//#endregion
...
const log$8 = larkLogger("card/image-resolver");
/** Matches complete markdown image syntax: `![alt](value)` */
const IMAGE_RE = /!\[([^\]]*)\]\(([^)\s]+)\)/g;
	...
	*/
	async resolveImagesAwait(text, timeoutMs) {
		...
		if (this.pending.size > 0) {
			log$8.info("resolveImagesAwait: waiting for uploads", {
			...
			if (this.pending.size > 0) log$8.warn("resolveImagesAwait: timed out with pending uploads", { remaining: this.pending.size });
		}
	...
	async doUpload(url) {
		try {
			log$8.info("uploading image", { url });
			...
			log$8.warn("image upload failed", {
				url,
				error: String(err)
			});
			this.pending.delete(url);
...
function recordToolUseEnd(params) {
	...
	const state = sessionTraces.get(sessionKey);
	if (!state) return;
	if (runId && state.currentRunId !== void 0 && state.currentRunId !== runId) return;
	const now = Date.now();
	...
	if (pendingIndex >= 0) {
		...
		step.error = error ? truncateText(error, 160) : void 0;
		step.durationMs = durationMs;
		step.finishedAt = now;
		if (!step.params && sanitizedParams) step.params = sanitizedParams;
		state.updatedAt = now;
		return;
	}
	state.steps.push({
		...
		error: error ? truncateText(error, 160) : void 0,
		durationMs,
...
function getToolUseTraceSteps(sessionKey) {
	...
	return state.steps.map((step) => {
		if (step.status === "running" && now - step.startedAt > STEP_RUNNING_TIMEOUT_MS) return {
			...step,
			status: "error",
			error: "timed out",
			finishedAt: now
...
function findPendingStepIndex(steps, toolName, params, toolCallId) {
	if (toolCallId) for (let index = steps.length - 1; index >= 0; index -= 1) {
...
function pruneTraceStore() {
	const now = Date.now();
	for (const [sessionKey, state] of sessionTraces) if (now - state.updatedAt > TRACE_TTL_MS) sessionTraces.delete(sessionKey);
	...
	for (const [sessionKey] of entries.slice(0, overflow)) sessionTraces.delete(sessionKey);
}
function sanitizeTraceValue(value, depth = 0, context = {}) {
	...
	if (Array.isArray(value)) return value.slice(0, 8).map((item) => sanitizeTraceValue(item, depth + 1, { source: context.source }));
	if (typeof value === "object") {
		const input = value;
		const output = {};
		for (const [key, entryValue] of Object.entries(input).slice(0, 12)) output[key] = isSensitiveKey(key) ? "[redacted]" : sanitizeTraceValue(entryValue, depth + 1, {
...
function sanitizeTraceString(value, context) {
	...
	if (isCommandLikeKey(context.key)) return redactInlineSecrets(redactedUrl);
	return redactedUrl;
}
function resolveStringLimit(context) {
	const key = context.key?.toLowerCase() ?? "";
	if (/(?:^|_)(?:command|script|description|prompt|task)(?:$|_)/.test(key)) return COMMAND_STRING_LIMIT;
	if (/(?:^|_)(?:path|file|url|uri|cwd|folder|dir)(?:$|_)/.test(key)) return PATH_STRING_LIMIT;
	if (context.source === "result") return RESULT_STRING_LIMIT;
...
function sortTraceValue(value) {
	...
	if (value && typeof value === "object") return Object.fromEntries(Object.entries(value).sort(([left], [right]) => left.localeCompare(right)).map(([key, entryValue]) => [key, sortTraceValue(entryValue)]));
	return value;
}
//#endregion
...
*/
const log$7 = larkLogger("card/unavailable-guard");
var UnavailableGuard = class {
	...
	*/
	terminate(source, err) {
		...
		log$7.warn("reply pipeline terminated by unavailable message", {
			source,
			apiCode,
...
*/
const log$6 = larkLogger("card/streaming");
var StreamingCardController = class {
	...
	async getFooterSessionMetrics() {
		try {
			...
			const defaultAgentId = resolveDefaultAgentId(this.deps.cfg);
			...
			if (sessionApi?.resolveStorePath && sessionApi?.loadSessionStore) {
				...
				if (!entry) {
					log$6.debug("footer metrics lookup: session entry missing", {
				...
				log$6.debug("footer metrics lookup: session entry found", {
					sessionKey: this.deps.sessionKey,
					matchedKey,
					storePath,
					source: "runtime.agent.session"
			...
			if (!channelSession?.resolveStorePath) return;
			...
			log$6.warn("footer metrics lookup failed", {
				error: String(err),
				sessionKey: this.deps.sessionKey
			});
			return;
	...
	*/
	...
	computeToolUseDisplay() {
		...
		return normalizeToolUseDisplay({
			traceSteps: getToolUseTraceSteps(this.deps.sessionKey),
	...
	get visibleToolUseElapsedMs() {
		if (!this.shouldDisplayToolUse || !this.toolUse.startedAt) return;
		return this.toolUse.elapsedMs || Date.now() - this.toolUse.startedAt;
	}
	computeToolUseTitleSuffix(display) {
		if (!this.shouldDisplayToolUse) return void 0;
		const stepCount = display?.stepCount ?? 0;
		return stepCount > 0 ? buildToolUseTitleSuffix({ stepCount }) : void 0;
	}
	/**
	...
	*/
	...
	transition(to, source, reason) {
		...
		if (!PHASE_TRANSITIONS[from].has(to)) {
			log$6.warn("phase transition rejected", {
				from,
				to,
				source
			});
	...
	markToolUseActivity() {
		if (!this.toolUse.startedAt) this.toolUse.startedAt = Date.now();
	...
	*/
	...
	async onPartialReply(payload) {
		...
		log$6.debug("onPartialReply", { len: text.length });
		if (!text) return;
		this.captureToolUseElapsed();
		if (!this.reasoning.reasoningStartTime) this.reasoning.reasoningStartTime = Date.now();
		...
		if (!this.text.streamingPrefix && SILENT_REPLY_TOKEN.startsWith(this.text.accumulatedText.trim())) {
			log$6.debug("onPartialReply: buffering NO_REPLY prefix");
			return;
		}
		await this.ensureCardCreated();
	...
	async onError(err, info) {
		...
		log$6.error(`${info.kind} reply failed`, { error: String(err) });
		this.captureToolUseElapsed();
		...
		const footerMetrics = this.needsFooterMetrics() ? await this.getFooterSessionMetrics() : void 0;
		...
		try {
			if (this.cardKit.cardMessageId) {
				if (idleEffectiveCardId) {
					...
					log$6.info("onIdle: closing streaming mode", {
				...
				if (!this.text.completedText && !this.text.accumulatedText) log$6.warn("reply completed without visible text, using empty-reply fallback");
				...
				if (idleEffectiveCardId) {
					...
					this.cardKit.cardKitSequence += 1;
					log$6.info("onIdle: updating final card", {
				...
				log$6.info("reply completed, card finalized", {
					elapsedMs: this.elapsed(),
					isCardKit: !!idleEffectiveCardId
				});
			}
		} catch (err) {
			log$6.warn("final card update failed", { error: String(err) });
		} finally {
			clearToolUseTraceRun(this.deps.sessionKey);
		}
	}
	markFullyComplete() {
		log$6.debug("markFullyComplete", {
	...
	async abortCard() {
		try {
			...
			if (effectiveCardId) {
				...
				log$6.info("abortCard completed", { effectiveCardId });
				...
				log$6.info("abortCard completed (IM fallback)", { messageId: this.cardKit.cardMessageId });
			}
		} catch (err) {
			log$6.warn("abortCard failed", { error: String(err) });
		} finally {
			clearToolUseTraceRun(this.deps.sessionKey);
		}
	}
			...
			try {
				try {
					...
					if (this.isStaleCreate(epoch)) {
						log$6.info("ensureCardCreated: stale epoch after createCardEntity, bailing out", {
							epoch,
							phase: this.phase
						});
						return;
					...
					if (cId) {
						...
						log$6.info("created CardKit entity", {
							cardId: cId,
						...
						if (this.isStaleCreate(epoch)) {
							log$6.info("ensureCardCreated: stale epoch after sendCardByCardId, bailing out", {
								epoch,
								phase: this.phase
							});
							this.disposeShutdownHook?.();
						...
						log$6.info("sent CardKit card", { messageId: result.messageId });
					...
					log$6.warn("CardKit flow failed, falling back to IM", { apiDetail });
					...
					if (this.isStaleCreate(epoch)) {
						log$6.info("ensureCardCreated: stale epoch after IM fallback send, bailing out", {
							epoch,
							phase: this.phase
						});
						return;
					...
					log$6.info("sent fallback IM card", { messageId: result.messageId });
				}
			} catch (err) {
				if (this.isStaleCreate(epoch)) return;
				if (this.guard.terminate("ensureCardCreated.outer", err)) return;
				log$6.warn("thinking card failed, falling back to static", { error: String(err) });
	...
	async performFlush() {
		...
		if (!this.cardKit.cardKitCardId && this.cardKit.originalCardKitCardId) {
			log$6.debug("performFlush: skipping (CardKit streaming disabled, awaiting final update)");
			return;
		}
		log$6.debug("flushCardUpdate: enter", {
		...
		try {
			...
			if (this.cardKit.cardKitCardId) {
				if (resolvedText !== this.text.lastFlushedText) {
					...
					this.cardKit.cardKitSequence += 1;
					log$6.debug("flushCardUpdate: answer seq bump", {
						seqBefore: prevSeq,
						seqAfter: this.cardKit.cardKitSequence
					});
				...
				log$6.debug("flushCardUpdate: IM patch fallback");
			...
			if (isCardRateLimitError(err)) {
				log$6.info("flushCardUpdate: rate limited (230020), skipping", { seq: this.cardKit.cardKitSequence });
				return;
			}
			if (isCardTableLimitError(err)) {
				log$6.warn("flushCardUpdate: card table limit exceeded (230099/11310), disabling CardKit streaming", { seq: this.cardKit.cardKitSequence });
				this.cardKit.cardKitCardId = null;
				return;
			}
			const apiDetail = extractApiDetail(err);
			log$6.error("card stream update failed", {
				apiCode,
				seq: this.cardKit.cardKitSequence,
				apiDetail
			});
			if (this.cardKit.cardKitCardId) {
				log$6.warn("disabling CardKit streaming, falling back to im.message.patch");
				this.cardKit.cardKitCardId = null;
			}
		}
	}
	...
	async updateToolUseStatus() {
		...
		try {
			...
			log$6.debug("updateToolUseStatus failed", { error: String(err) });
		}
	}
	...
	*/
	async closeStreamingAndUpdate(cardId, card, label) {
		const seqBeforeClose = this.cardKit.cardKitSequence;
		this.cardKit.cardKitSequence += 1;
		log$6.info(`${label}: closing streaming mode`, {
			seqBefore: seqBeforeClose,
			seqAfter: this.cardKit.cardKitSequence
		});
		await setCardStreamingMode({
		...
		const seqBeforeUpdate = this.cardKit.cardKitSequence;
		this.cardKit.cardKitSequence += 1;
		log$6.info(`${label}: updating card`, {
			seqBefore: seqBeforeUpdate,
			seqAfter: this.cardKit.cardKitSequence
		});
		await updateCardKitCard({
...
*/
function prepareTerminalCardContent(content, imageResolver, tableLimit = 3) {
	const resolvedReasoningText = content.reasoningText ? imageResolver.resolveImages(content.reasoningText) : void 0;
	const resolvedText = imageResolver.resolveImages(content.text);
	const sanitizedSegments = sanitizeTextSegmentsForCard(resolvedReasoningText ? [resolvedReasoningText, resolvedText] : [resolvedText], tableLimit);
	if (resolvedReasoningText) return {
...
*/
const log$5 = larkLogger("card/reply-dispatcher");
function createFeishuReplyDispatcher(params) {
	...
	log$5.info("reply mode resolved", {
		effectiveReplyMode,
		replyMode,
		chatType
	});
	log$5.info("footer config resolved", {
		accountId: account.accountId,
		sessionKey,
		chatType,
		useStreamingCards,
	...
	const typingCallbacks = createTypingCallbacks({
		...
		start: async () => {
			...
			if (typingStopped && typingState) {
				...
				typingState = null;
				log$5.info("removed typing indicator (raced with stop)");
				return;
			}
			log$5.info("added typing indicator reaction");
		},
		stop: async () => {
			typingStopped = true;
			...
			typingState = null;
			log$5.info("removed typing indicator reaction");
		},
		onStartError: (err) => {
			logTypingFailure({
				log: (message) => log$5.warn(message),
				channel: "feishu",
				action: "start",
				error: err
			});
	...
	const { dispatcher, replyOptions, markDispatchIdle } = core.channel.reply.createReplyDispatcherWithTyping({
		...
		deliver: async (payload, meta) => {
			...
			if (staticAborted || controller?.isTerminated || controller?.isAborted) {
				log$5.debug("deliver: skipped (aborted)");
				return;
			}
			if (dispatchFullyComplete) {
				log$5.debug("deliver: skipped (dispatch already complete)");
				return;
			}
			...
			if (!text.trim() && payloadMediaUrls.length === 0) {
				log$5.debug("deliver: empty text and no media, skipping");
				return;
			}
			if (controller) {
				...
				if (text.trim()) {
					...
					if (controller.cardMessageId) {
						await controller.onDeliver({
							...payload,
							text
						});
						return;
					}
					log$5.warn("deliver: card creation failed, falling back to static delivery");
				}
			}
			if (text.trim()) if (shouldUseCard(text)) {
				const chunks = core.channel.text.chunkTextWithMode(text, textChunkLimit, chunkMode);
				log$5.info("deliver: sending card chunks", {
					count: chunks.length,
					chatId
				});
				...
				for (const chunk of chunks) {
					...
					try {
						...
						if (isCardTableLimitError(err)) {
							log$5.warn("card table limit exceeded (230099/11310), falling back to text", { chatId });
				...
				log$5.info("deliver: sending text chunks", {
					count: chunks.length,
					chatId
				});
			...
			for (const mediaUrl of payloadMediaUrls) {
				...
				try {
					log$5.info("deliver: sending media via static path", { mediaUrl: mediaUrl.slice(0, 80) });
					await sendMediaLark({
						cfg,
						to: chatId,
						mediaUrl,
					...
					log$5.error("deliver: static media send failed", { error: String(mediaErr) });
				}
			}
		},
		onError: async (err, info) => {
			...
			log$5.error(`${info.kind} reply failed`, { error: String(err) });
			typingCallbacks.onIdle?.();
		},
		onIdle: async () => {
...
function getVisiblePayloadText(payload) {
	...
	if (!rawText) return "";
	const split = splitReasoningText(rawText);
	if (split.answerText != null) return split.answerText;
...
function shouldRouteToolPayloadToCard(payload, showToolUse) {
	...
	if (execApproval && typeof execApproval === "object" && !Array.isArray(execApproval)) return false;
	return true;
}
//#endregion
//#region src/card/tool-use-config.ts
...
function resolveEffectiveVerboseMode(params) {
	return extractInlineVerboseMode(params.body) ?? resolveSessionVerboseMode(params.cfg, params.sessionKey, params.agentId) ?? normalizeToolUseMode(params.cfg.agents?.defaults?.verboseDefault) ?? "off";
}
function resolveSessionVerboseMode(cfg, sessionKey, agentId) {
	try {
		const cfgWithSession = cfg;
		const store = loadSessionStore(resolveStorePath(cfgWithSession.session?.store ?? cfgWithSession.sessions?.store, { agentId }));
		...
		for (const candidateKey of candidateKeys) {
			...
			if (mode) return mode;
			if (resolved.existing) return void 0;
		}
		return;
	} catch {
		return;
...
function extractInlineVerboseMode(body) {
	...
	let last;
	for (const match of matches) last = normalizeToolUseMode(match[1]);
	return last;
}
function normalizeToolUseMode(value) {
	if (typeof value !== "string") return void 0;
	const normalized = value.trim().toLowerCase();
	if (normalized === "off" || normalized === "on" || normalized === "full") return normalized;
}
//#endregion
...
const log$4 = larkLogger("inbound/dispatch-commands");
/**
...
async function dispatchSystemCommand(dc, ctxPayload, replyToMessageId) {
	...
	log$4.info("system command detected, plain-text dispatch");
	await dc.core.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
		ctx: ctxPayload,
		cfg: dc.accountScopedCfg,
		dispatcherOptions: {
			deliver: async (payload, info) => {
				if (suppressToolDetails && info.kind === "tool") return;
	...
	log$4.info(`system command dispatched (delivered=${delivered}, elapsed=${ticketElapsed()}ms)`);
}
...
const log$3 = larkLogger("inbound/dispatch-context");
/**
...
function ensureRuntime(runtime) {
	...
	return {
		log: (...args) => log$3.info(args.map(String).join(" ")),
		error: (...args) => log$3.error(args.map(String).join(" ")),
...
async function resolveThreadSessionKey(params) {
	...
	if (!await isThreadCapableGroup({
		...
		log$3.info(`thread session skipped: group ${chatId} is not topic/thread mode`);
		return;
	}
...
const log$2 = larkLogger("inbound/dispatch");
/**
...
async function dispatchCommentMessage(dc, ctxPayload, skillFilter) {
	...
	log$2.info(`dispatching comment reply (session=${effectiveSessionKey})`);
	let delivered = false;
	await dc.core.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
		ctx: ctxPayload,
		cfg: dc.accountScopedCfg,
		dispatcherOptions: {
			...
			onSkip: (_payload, info) => {
				if (info.reason !== "silent") dc.log(`feishu[${dc.account.accountId}]: comment reply skipped (reason=${info.reason})`);
			},
			onError: (err, info) => {
				dc.error(`feishu[${dc.account.accountId}]: comment ${info.kind} reply failed: ${String(err)}`);
			}
		},
		replyOptions: { ...skillFilter ? { skillFilter } : {} }
	});
	dc.log(`feishu[${dc.account.accountId}]: comment dispatch complete (delivered=${delivered})`);
	log$2.info(`comment dispatch complete (delivered=${delivered}, elapsed=${ticketElapsed()}ms)`);
}
async function dispatchNormalMessage(dc, ctxPayload, chatHistories, historyKey, historyLimit, replyToMessageId, skillFilter, skipTyping) {
	...
	if (isLikelyAbortText(dc.ctx.content?.trim() ?? "")) {
		...
		log$2.info("abort message detected, using plain-text dispatch");
	...
	if (toolUseDisplay.showToolUse) startToolUseTraceRun(effectiveSessionKey);
	else clearToolUseTraceRun(effectiveSessionKey);
	...
	log$2.info(`dispatching to agent (session=${effectiveSessionKey})`);
	try {
		...
		log$2.info(`dispatch complete (replies=${counts.final}, elapsed=${ticketElapsed()}ms)`);
	} finally {
...
async function dispatchToAgent(params) {
	...
	if (!dc.isThread && dc.isGroup && dc.ctx.rootId && dc.account.config?.threadSession === true) {
		if (await isThreadCapableGroup({
			...
			log$2.info(`inferred thread from root_id=${dc.ctx.rootId} in topic group ${dc.ctx.chatId}`);
			dc.isThread = true;
			dc.ctx = {
				...dc.ctx,
				threadId: dc.ctx.rootId
			};
		}
	}
	...
	if (params.permissionError && !isCommentTarget(dc.ctx.chatId)) try {
	...
	const ctxPayload = buildInboundPayload(dc, {
		...
		wasMentioned: mentionedBot(params.ctx) || params.ctx.mentionAll && resolveRespondToMentionAll({
	...
	if (i18nCommandName) {
		...
		log$2.info(`${i18nCommandName} command detected, using i18n dispatch`);
		try {
			let i18nTexts;
	...
	const isCommand = !isCommentFlow && dc.core.channel.commands.isControlCommandMessage(params.ctx.content, params.accountScopedCfg);
...
const logger$3 = larkLogger("inbound/handler");
async function handleFeishuMessage(params) {
	...
	const log = runtime?.log ?? ((...args) => logger$3.info(args.map(String).join(" ")));
	const error = runtime?.error ?? ((...args) => logger$3.error(args.map(String).join(" ")));
	...
	logger$3.info(`received from ${ctx.senderId} in ${ctx.chatId} (${ctx.chatType})`);
	...
	if (!gate.allowed) {
		if (gate.reason === "no_mention") logger$3.info(`rejected: no bot mention in group ${ctx.chatId}`);
	...
	try {
		...
		logger$3.error(`dispatch failed: ${String(err)} (elapsed=${ticketElapsed()}ms)`);
	}
}
injectInboundHandler(handleFeishuMessage);
//#endregion
//#region src/tools/ask-user-question.ts
/**
* Copyright (c) 2026 ByteDance Ltd. and/or its affiliates
* SPDX-License-Identifier: MIT
*
...
*/
const byChatContext = /* @__PURE__ */ new Map();
...
function armTtlTimer(ctx, delayMs) {
	...
	ctx.ttlTimer = setTimeout(() => {
		if (!pendingQuestions.has(ctx.questionId)) return;
		if (ctx.submitted) return;
		log$1.info(`question ${ctx.questionId} expired (TTL ${delayMs}ms)`);
		consumePendingQuestion(ctx.questionId);
		setImmediate(async () => {
			try {
				await updateCardToExpired(ctx);
			} catch (err) {
				log$1.warn(`failed to update card to expired state: ${err}`);
			}
		});
	}, delayMs);
}
...
function findQuestionByChat(accountId, chatId) {
	...
	for (const qid of set) {
		...
		if (ctx && !ctx.submitted) {
			if (match) {
				log$1.warn(`chat-scoped fallback ambiguous: multiple pending questions in ${baseKey}`);
				return;
			}
			match = ctx;
		}
...
function handleAskUserAction(data, _cfg, accountId) {
	...
	try {
		...
		log$1.info(`card action received: tag=${actionTag}, name=${actionName}, chat=${openChatId}, sender=${senderOpenId}, hasFormValue=${!!formValue}, hasValue=${!!event.action?.value}`);
		...
		if (!action && actionName?.startsWith(SUBMIT_BUTTON_PREFIX)) {
			...
			if (!operationId) operationId = actionName.slice(16);
		}
		if (!action && actionTag === "button" && formValue) action = ACTION_SUBMIT;
		if (!action && actionTag === "form_submit") {
			action = ACTION_SUBMIT;
			if (!formValue && event.action) formValue = event.action;
		}
	} catch {
		return;
	}
	if (action !== ACTION_SUBMIT) return void 0;
	let ctx;
	if (operationId) ctx = pendingQuestions.get(operationId);
	if (!ctx && openChatId) {
		ctx = findQuestionByChat(accountId, openChatId);
		if (ctx) log$1.info(`resolved question via chat-scoped fallback: ${ctx.questionId}`);
	}
	if (!ctx) {
		if (operationId) log$1.warn(`ask-user action: question ${operationId} not found (expired or already handled)`);
		return operationId ? { toast: {
			type: "info",
			content: "该问题已过期或已被回答"
		} } : void 0;
	}
	if (ctx.submitted) return { toast: {
		type: "info",
		content: "该问题已提交，请等待处理"
	} };
	if (senderOpenId && ctx.senderOpenId && senderOpenId !== ctx.senderOpenId) return { toast: {
		type: "warning",
		content: "只有被提问的用户可以回答此问题"
	} };
	if (!formValue) {
		log$1.warn(`ask-user submit without form_value for question ${operationId}`);
		return { toast: {
			type: "error",
			content: "表单数据丢失，请重试"
		} };
	}
	log$1.info(`form_value: ${JSON.stringify(formValue)}`);
	const answers = {};
	const unanswered = [];
	for (let i = 0; i < ctx.questions.length; i++) {
		const q = ctx.questions[i];
		let answer;
		if (q.options.length === 0) answer = readFormTextField(formValue, getInputFieldName(i));
		else if (q.multiSelect) {
			const selected = readFormMultiSelect(formValue, getSelectFieldName(i));
			if (selected.length > 0) answer = selected.join(", ");
		} else answer = readFormTextField(formValue, getSelectFieldName(i));
		if (answer) answers[q.question] = answer;
		else unanswered.push(q.header);
	}
	if (unanswered.length > 0) return { toast: {
		type: "warning",
		content: `请先完成: ${unanswered.join(", ")}`
	} };
	ctx.submitted = true;
	const processingCard = buildProcessingCard(ctx.questions, answers);
	setImmediate(() => {
		injectAnswerSyntheticMessage(ctx, answers).catch((err) => {
			log$1.error(`unhandled error in injectAnswerSyntheticMessage: ${err}`);
		});
	});
	log$1.info(`question ${operationId} submitted, synthetic message will be injected`);
	return {
		toast: {
			type: "success",
			content: "已收到回答，正在处理..."
...
async function injectAnswerSyntheticMessage(ctx, answers) {
	...
	try {
		...
		log$1.warn(`failed to update card to processing state via API: ${err}`);
	}
	let lastError;
	for (let attempt = 0; attempt <= INJECT_MAX_RETRIES; attempt++) {
		if (attempt > 0) {
			log$1.info(`retrying synthetic message injection (attempt ${attempt + 1}) for question ${ctx.questionId}`);
		...
		try {
			...
			await promise;
			consumePendingQuestion(ctx.questionId);
			log$1.info(`synthetic answer message dispatched (${status}) for question ${ctx.questionId}`);
			try {
				await updateCardToAnswered(ctx, answers);
			} catch (err) {
				log$1.warn(`failed to update card to answered state: ${err}`);
			}
			return;
		} catch (err) {
			lastError = err;
			log$1.warn(`synthetic message injection attempt ${attempt + 1} failed: ${err}`);
		}
	}
	ctx.submitted = false;
	armTtlTimer(ctx, PENDING_QUESTION_TTL_MS);
	log$1.error(`synthetic message injection failed after ${INJECT_MAX_RETRIES + 1} attempts for question ${ctx.questionId}: ${lastError}`);
	try {
		await updateCardToSubmittable(ctx);
		log$1.info(`reverted card to submittable state for question ${ctx.questionId}`);
	} catch (err) {
		log$1.warn(`failed to revert card to submittable state: ${err}`);
	}
}
...
function readFormMultiSelect(formValue, fieldName) {
	...
	if (Array.isArray(raw)) return raw.filter((v) => typeof v === "string" && v.trim().length > 0);
...
function buildQuestionFormElements(q, questionIndex) {
	...
	if (q.question && q.question !== q.header) elems.push({
		tag: "markdown",
		content: q.question,
		text_size: "notation"
	});
	...
	if (q.multiSelect) elems.push(buildLabeledRow(labelMd, {
		...
		placeholder: {
			...
			i18n_content: {
				zh_cn: "请选择...",
				en_us: "Select options..."
			}
		},
		options: selectOptions
	}));
	else elems.push(buildLabeledRow(labelMd, {
		tag: "select_static",
		name: getSelectFieldName(questionIndex),
		placeholder: {
			tag: "plain_text",
	...
	if (descLines.length > 0) elems.push({
		tag: "markdown",
...
const AskUserQuestionSchema = Type.Object({ questions: Type.Array(Type.Object({
	...
	options: Type.Array(Type.Object({
		...
		description: "Available choices. Renders as a dropdown. Leave empty ([]) for free-text input — the user will see a text field instead.",
		maxItems: 10
	}),
	multiSelect: Type.Boolean({ description: "Whether multiple options can be selected (ignored when options is empty)" })
}), {
...
function registerAskUserQuestionTool(api) {
	...
	api.registerTool({
		...
		description: "Ask the user a question via an interactive Feishu card. Returns immediately after sending the card. The user's answers will arrive as a new message in the conversation. Do NOT poll or re-call this tool — just wait for the response message. For selection questions, provide options (renders as dropdown). For free-text input, set options to an empty array.",
		parameters: AskUserQuestionSchema,
		async execute(_toolCallId, params) {
			const { questions } = params;
			const ticket = getTicket();
			if (!ticket) return formatToolError("AskUserQuestion can only be used in a Feishu message context");
			const { chatId, accountId, senderOpenId, threadId } = ticket;
			if (!senderOpenId) return formatToolError("Cannot determine the target user (no senderOpenId in ticket)");
			const questionId = randomUUID();
			log$1.info(`creating ask-user-question: id=${questionId}, questions=${questions.length}, chat=${chatId}`);
			const card = buildAskUserCard(questions, questionId);
			let cardId;
			try {
				cardId = await createCardEntity({
					cfg,
					card,
					accountId
				});
			} catch (err) {
				log$1.error(`failed to create card entity: ${err}`);
				return formatToolError(`Failed to create question card: ${err}`);
			}
			if (!cardId) return formatToolError("Failed to create question card: no card_id returned");
			try {
				await sendCardByCardId({
					cfg,
					to: chatId,
				...
				log$1.error(`failed to send card: ${err}`);
			...
			log$1.info(`question ${questionId} card sent, returning pending status`);
			return formatToolResult({
				status: "pending",
				questionId,
				message: "Question card sent to the user. Their answers will arrive as a follow-up message in this conversation. Do NOT call this tool again for the same question — just wait for the response message."
			});
		}
	});
...
*/
const logger$2 = larkLogger("inbound/reaction-handler");
...
async function resolveReactionContext(params) {
	...
	const log = runtime?.log ?? ((...args) => logger$2.info(args.map(String).join(" ")));
	const account = getLarkAccount(cfg, accountId);
...
async function handleFeishuReaction(params) {
	...
	const log = runtime?.log ?? ((...args) => logger$2.info(args.map(String).join(" ")));
	const error = runtime?.error ?? ((...args) => logger$2.error(args.map(String).join(" ")));
	const emojiType = event.reaction_type?.emoji_type ?? "";
	const messageId = event.message_id;
	const operatorOpenId = event.user_id?.open_id ?? "";
	const account = getLarkAccount(cfg, accountId);
	...
	logger$2.info(`reaction "${emojiType}" by ${operatorOpenId} on ${messageId} (chatType=${preResolved.chatType})`);
	const isGroup = ctx.chatType === "group";
	const groupConfig = isGroup ? resolveFeishuGroupConfig({
		cfg: accountFeishuCfg,
		groupId: ctx.chatId
...
//#endregion
//#region src/messaging/inbound/comment-context.ts
const logger$1 = larkLogger("inbound/comment-context");
/**
...
function inferIsWholeComment(params) {
	if (typeof params.explicitIsWhole === "boolean") return params.explicitIsWhole;
...
async function fetchDocTitle(params) {
	...
	try {
		return ((await LarkClient.fromCfg(cfg, accountId).sdk.drive.v1.fileMeta.batchQuery({ data: {
			request_docs: [{
				doc_token: fileToken,
				doc_type: fileType
			}],
			with_url: false
		} }))?.data?.metas?.[0])?.title || void 0;
	} catch (err) {
		logger$1.warn(`failed to fetch doc title: ${err}`);
		return;
	}
}
/**
...
async function fetchComment(params) {
	...
	try {
		...
		while (commentHasMore && !comment && commentPages < MAX_COMMENT_PAGES) {
			commentPages++;
			const data = (await client.sdk.drive.v1.fileComment.list({
			...
			comment = (data?.items ?? []).find((c) => c.comment_id === commentId);
		...
		let hasMore = true;
		while (hasMore) {
			const replyData = (await client.sdk.drive.v1.fileCommentReply.list({
				path: {
		...
		logger$1.warn(`failed to fetch comment: ${err}`);
		return;
	}
}
/**
...
async function resolveDriveCommentEventTurn(params) {
	...
	if (!fileToken || !commentId) {
		logger$1.warn("missing file_token or comment_id in comment event");
		return null;
	}
	const [docTitle, commentData] = await Promise.all([fetchDocTitle({
		cfg,
		fileToken,
		fileType,
		accountId
	...
	if (commentData) {
		if (commentData.comment?.quote) quotedText = String(commentData.comment.quote);
		...
		if (event.reply_id && commentData.replies.length > 0) {
			...
			commentText = targetReply ? extractReplyText(targetReply) : void 0;
			const chainReplies = commentData.replies.filter((r) => r.reply_id !== event.reply_id);
			if (chainReplies.length > 0) replyChainContext = chainReplies.map((r) => {
				return `[${r.user_id?.open_id ?? "unknown"}]: ${extractReplyText(r)}`;
			}).join("\n");
		} else {
			const rootReply = commentData.comment?.reply_list?.replies?.[0];
			commentText = rootReply ? extractReplyText(rootReply) : void 0;
		}
	}
	...
	if (commentData) {
		const rootReply = commentData.comment?.reply_list?.replies?.[0];
		rootCommentText = rootReply ? extractReplyText(rootReply) : void 0;
	}
...
//#endregion
//#region src/messaging/inbound/comment-handler.ts
/**
* Copyright (c) 2026 ByteDance Ltd. and/or its affiliates
* SPDX-License-Identifier: MIT
*
...
async function handleFeishuCommentEvent(params) {
	...
	if (dmPolicy !== "open") {
		...
		const userIdMatch = senderUserId ? resolveFeishuAllowlistMatch({
			allowFrom: combinedAllowFrom,
		...
		if (!match.allowed && !userIdMatch.allowed) {
			if (dmPolicy === "pairing") {
				...
				try {
					...
					try {
						await LarkClient.fromCfg(accountScopedCfg, accountId).sdk.request({
							method: "POST",
							...
							data: { content: { elements: [{
								type: "text_run",
								text_run: { text: pairingText }
							}] } }
						});
						sentInThread = true;
					} catch {}
					if (!sentInThread) await sendPairingReply({
						senderId: senderOpenId,
						chatId: senderOpenId,
						accountId: account.accountId,
						accountScopedCfg
			...
			} else log(`feishu[${accountId}]: comment event rejected (dmPolicy=${dmPolicy}, not in allowlist)`);
			return;
		}
	}
	...
	if (eventMentioned !== true && !textMentioned) {
		log(`feishu[${accountId}]: comment event not mentioning bot, skipping (eventFlag=${String(eventMentioned)}, textMention=${textMentioned})`);
		return;
	}
	...
	log(`feishu[${accountId}]: comment event on ${commentId}${event.reply_id ? ` (reply ${event.reply_id})` : ""}, dispatching to agent`);
...
//#endregion
//#region src/channel/interactive-dispatch.ts
...
async function dispatchFeishuPluginInteractiveHandler(params) {
	...
	if (!basics) return void 0;
	if (!basics.action) return void 0;
	const respond = {
		reply: async (args) => {
		...
		editMessage: async (args) => {
			if (!basics.openMessageId) {
				...
				if (typeof args?.text === "string" && args.text.trim() && basics.openChatId) await sendMessageFeishu({
					cfg: params.cfg,
					to: basics.openChatId,
					text: args.text,
					replyToMessageId: basics.openMessageId,
	...
	try {
		...
		if (!(await dispatchPluginInteractiveHandler({
			...
			invoke: async (match) => {
				...
				return { handled: cardResponse !== void 0 };
			}
		})).matched) return void 0;
		return cardResponse;
	} catch (err) {
...
async function handleMessageEvent(ctx, data) {
	...
	try {
		...
		const threadId = event.message?.thread_id || event.message?.root_id || void 0;
...
async function handleCommentEvent(ctx, data) {
	...
	try {
		...
		parsed.user_id?.open_id;
		const isMentioned = parsed.is_mention ?? false;
		const eventTimestamp = parsed.action_time;
		log(`feishu[${accountId}]: drive comment event: type=${parsed.file_type}, comment=${commentId}${replyId ? `, reply=${replyId}` : ""}${isMentioned ? ", @bot" : ""}`);
...
async function handleCardActionEvent(ctx, data) {
	try {
		...
		if (askResult !== void 0) return askResult;
		const authResult = await handleCardAction(data, ctx.cfg, ctx.accountId);
		if (authResult !== void 0) return authResult;
...
//#endregion
export { sendCommentReplyLark as $, resolveAnyEnabledToolsConfig as A, sendMessageFeishu as B, triggerOnboarding as C, getTicket as Ct, formatToolResult as D, getLarkAccount as Dt, createToolContext as E, getEnabledLarkAccounts as Et, getStoredToken as F, formatMentionAllForCard as G, buildMentionedCardContent as H, filterSensitiveScopes as I, formatMentionForText as J, formatMentionAllForText as K, getAppGrantedScopes as L, mcpDomain as M, openPlatformDomain as N, getFirstAccount as O, getLarkAccountIds as Ot, wwwDomain as P, sendCardLark as Q, editMessageFeishu as R, handleInvokeErrorWithAutoAuth as S, larkLogger as St, registerFeishuOAuthTool as T, getDefaultLarkAccountId as Tt, buildMentionedMessage as U, updateCardFeishu as V, extractMessageBody as W, nonBotMentions as X, mentionedBot as Y, resolveFeishuGroupToolPolicy as Z, getMessageFeishu as _, normalizeFeishuTarget as _t, registerAskUserQuestionTool as a, uploadAndSendMediaLark as at, buildConvertContextFromItem as b, LarkClient as bt, formatDiagReportCli as c, validateLocalMediaRoots as ct, traceByMessageId as d, AppScopeCheckFailedError as dt, sendMediaLark as et, probeFeishu as f, AppScopeMissingError as ft, checkMessageGate as g, looksLikeFeishuId as gt, sanitizeParamsForLog as h, isCommentTarget as ht, isMessageExpired as i, sendImageLark as it, rawLarkRequest as j, registerTool as k, formatTraceOutput as l, assertLarkOk as lt, recordToolUseStart as m, UserScopeInsufficientError as mt, monitor_exports as n, sendAudioLark as nt, registerCommands as o, uploadFileLark as ot, recordToolUseEnd as p, UserAuthRequiredError as pt, formatMentionForCard as q, handleFeishuReaction as r, sendFileLark as rt, analyzeTrace as s, uploadImageLark as st, monitorFeishuProvider as t, sendTextLark as tt, runDiagnosis as u, formatLarkError as ut, parseMessageEvent as v, parseFeishuRouteTarget as vt, executeAuthorize as w, createAccountScopedConfig as wt, extractMentionOpenId as x, getUserAgent as xt, convertMessageContent as y, resolveReceiveIdType as yt, sendCardFeishu as z };
